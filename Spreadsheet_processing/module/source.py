...
import os
import openpyxl
os.chdir('C:\\Python27\SPREADSHEET_PROCESSING')
inbook=openpyxl.load_workbook('export.xlsx')
outbook=openpyxl.load_workbook('template.xlsx')
...
for i in range(2,insheet.max_row+1):
    ...
    outsheet.cell(row=i+x,column=3).value=insheet.cell(row=i,column=5).value  #Ticket Priority
